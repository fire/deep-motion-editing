...
class IntegratedModel:
    ...
    def __init__(
        ...
        self.edges = build_edge_topology(
            joint_topology, torch.zeros((len(joint_topology), 3))
        )
        ...
        if not args.use_sep_ee:
            self.height.unsqueeze_(-1)
        if args.ee_loss_fact == "learn":
            self.height_para = [self.height]
        else:
            self.height_para = []
    ...
    def G_parameters(self):
        return (
            list(self.auto_encoder.parameters())
            + list(self.static_encoder.parameters())
            + self.height_para
        )
    ...
    def save(self, path, epoch):
        ...
        torch.save(
            self.auto_encoder.state_dict(), os.path.join(path, "auto_encoder.pt")
        )
        torch.save(
            self.discriminator.state_dict(), os.path.join(path, "discriminator.pt")
        )
        torch.save(
            self.static_encoder.state_dict(), os.path.join(path, "static_encoder.pt")
        )
