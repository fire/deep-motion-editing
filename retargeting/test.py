    ...
    for f in files:
        if dest_suffix is not None:
            cmd = 'cp "{}" "{}"'.format(
                ...
                os.path.join(dest_path, f[:-length] + "_{}.bvh".format(dest_suffix)),
            )
        else:
            cmd = 'cp "{}" "{}"'.format(
                os.path.join(source_path, f),
                os.path.join(dest_path, f[:-length] + ".bvh"),
            )
        os.system(cmd)
    ...
    for i in range(4):
        ...
        if i == 0:
            ...
            for char in test_characters:
                batch_copy(
                    os.path.join(source_path, char),
                    0,
                    os.path.join(cross_dest_path, char),
                )
                batch_copy(
                    os.path.join(source_path, char),
                    "gt",
        ...
        for char in test_characters:
            for char in test_characters:
                batch_copy(
                    os.path.join(source_path, char), 1, os.path.join(intra_dest, char)
                )
                batch_copy(
                    os.path.join(source_path, char),
                    "gt",
                    os.path.join(intra_dest, char),
                    "gt",
                )
