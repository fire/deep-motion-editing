...
class SkeletonConv(nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, kernel_size, joint_num, stride=1, padding=0,
        ...
        for i, neighbour in enumerate(self.expanded_neighbour_list):
            self.mask[self.out_channels_per_joint *
                      i: self.out_channels_per_joint * (i + 1), neighbour, ...] = 1
    ...
    def reset_parameters(self):
        for i, neighbour in enumerate(self.expanded_neighbour_list):
            ...
            if self.bias is not None:
                ...
                self.bias[self.out_channels_per_joint *
                          i: self.out_channels_per_joint * (i + 1)] = tmp
...
class SkeletonPool(nn.Module):
    def __init__(self, edges, pooling_mode, channels_per_edge, last_pool=False):
        ...
        if pooling_mode != 'mean':
            raise Exception(
                'Unimplemented pooling mode in matrix_implementation')
        ...
        self.weight = torch.zeros(
            len(self.pooling_list) * channels_per_edge, self.edge_num * channels_per_edge)

        for i, pair in enumerate(self.pooling_list):
            for j in pair:
                for c in range(channels_per_edge):
                    self.weight[i * channels_per_edge + c, j *
                                channels_per_edge + c] = 1.0 / len(pair)
...
class SkeletonUnpool(nn.Module):
    def __init__(self, pooling_list, channels_per_edge):
        ...
        weight_size = self.output_edge_num * channels_per_edge, self.input_edge_num * channels_per_edge
        self.weight = torch.zeros(weight_size)
    ...
    for k in range(edge_num):
        for i in range(edge_num):
            for j in range(edge_num):
                edge_mat[i][j] = min(
                    edge_mat[i][j], edge_mat[i][k] + edge_mat[k][j])
    return edge_mat
...
def find_neighbor(edges, d):
    ...
    global_part_neighbor = []
    for n, neighbor in enumerate(neighbor_list):
        if 0 in neighbor:
            global_part_neighbor.append(n)
