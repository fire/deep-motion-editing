...
class GAN_model(BaseModel):
    def __init__(self, args, character_names, dataset):
        ...
        self.G_para = []
        self.args = args
        print(f"topology {self.n_topology}")
        for n_topo in range(self.n_topology):
            print(character_names[n_topo][0])
            print(f"joint_topologies len {len(dataset.joint_topologies[n_topo])}")
            model = IntegratedModel(
                args, dataset.joint_topologies[n_topo], None, self.device, character_names[n_topo][0]
            )
