...
class IntegratedModel:
    ...
    def __init__(
        self, args, joint_topology, origin_offsets: torch.Tensor, device, char
    ):
        self.args = args
    ...
    def load(self, path, epoch=None):
        ...
        self.auto_encoder.load_state_dict(
            torch.load(
                pjoin(path, "auto_encoder.pt"), map_location=self.args.cuda_device
            )
        )
        self.static_encoder.load_state_dict(
            torch.load(
                pjoin(path, "static_encoder.pt"), map_location=self.args.cuda_device
            )
        )
        print("load succeed!")
