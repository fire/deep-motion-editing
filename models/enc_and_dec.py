...
class Decoder(nn.Module):
    def __init__(self, args, enc: Encoder):
        ...
        for i in range(args.num_layers):
            ...
            self.unpools.append(
                SkeletonUnpool(
                    enc.pooling_list[args.num_layers - i - 1],
                    in_channels // len(neighbor_list),
                )
            )

            seq.append(
                nn.Upsample(scale_factor=2, mode=args.upsampling, align_corners=False)
            )
...
class StaticEncoder(nn.Module):
    def __init__(self, args, edges):
        ...
        for i in range(args.num_layers):
            ...
            if i < args.num_layers - 1:
                pool = SkeletonPool(
                    edges, channels_per_edge=channels * 2, pooling_mode="mean"
                )
                seq.append(pool)
