...
def motion_projection(motion, local_x, view_angle=None):
    ...
    motion_proj = (
        local[..., np.newaxis, np.newaxis, [0, 1], :] @ motion
    )  # [(B,), 1, 1, 2, 3] @ [(B,), T, J, 3, 1] => [(B,), T, J, 2, 1]
...
def foot_contact_from_positions(positions, fid_l=(3, 4), fid_r=(7, 8)):
    ...
    for fid_index in [fid_l, fid_r]:
        foot_vel = (
            positions[1:, fid_index] - positions[:-1, fid_index]
        ) ** 2  # [T - 1, 2, 3]
...
def phase_from_ft(foot_contact, is_debug=False):
    ...
    if is_debug:
        print(
            "%d circles, total length = %d, avg length = %.3lf"
            % (num_circles, circle_length, avg_circle)
        )
...
def y_rotation_from_positions(positions, hips=(2, 6), sdrs=(14, 18)):
    ...
    quaters = Quaternions.between(forward, target)[
        ..., np.newaxis, :
    ]  # [T, 4] -> [T, 1, 4]
    pivots = Pivots.from_quaternions(
        -quaters
    ).ps  # from "target"[0, 0, 1] to current facing direction "forward"
...
class AnimationData:
    ...
    def __init__(self, full, skel=None, frametime=1 / 30):
        ...
        assert self.rotations.shape[1] == len(
            self.skel.topology
        ), "Rotations do not match the skeleton."
        ...
        self.full = np.concatenate(
            [
                self.rotations.reshape(self.len, -1),
                self.rt_pos,
                self.rt_rot,
                self.foot_contact,
            ],
        ...
        self.local_x = None  # [3]
        self.positions_for_proj = (
            None  # [T, (J - 1) + 1, 3], trimmed and not forward facing
        )
    ...
    def get_original_rotations(self, rt_rot=None):
        ...
        return np.concatenate(
            (rt_rotations, self.rotations[:, 1:]), axis=1
        )  # [T, J, 4]
    ...
    def get_content_input(self):
        ...
        return np.concatenate((rotations, self.rt_pos, self.rt_rot), axis=-1).transpose(
            1, 0
        )  # [Jo * 4 + 3 + 1, T]

    def get_style3d_input(self):
        pos3d = forward_rotations(self.skel, self.rotations, trim=True)[
            :, 1:
        ]  # [T, J - 1, 3]
        pos3d = pos3d.reshape((len(pos3d), -1))  # [T, (J - 1) * 3]
        return np.concatenate((pos3d, self.rt_pos, self.rt_rot), axis=-1).transpose(
            1, 0
        )  # [(J - 1) * 3 + 3 + 1, T]
    ...
    def get_projections(self, view_angles, scales=None):
        if self.positions_for_proj is None:
            ...
            positions = forward_rotations(self.skel, rotations, trim=True)[
                :, 1:
            ]  # [T, J - 1, 3]
            positions = np.concatenate(
                (positions, self.rt_pos[:, np.newaxis, :]), axis=1
            )  # [T, J, 3]
        ...
        for angle, scale in zip(view_angles, scales):
            projections.append(
                motion_projection(positions, self.get_local_x(), angle) * scale
            )
    ...
    def get_global_positions(self, trim=True):  # for visualization
        if not trim:
            return forward_rotations(
                self.skel, self.get_original_rotations(), rtpos=self.rt_pos, trim=False
            )
        if self.global_positions is None:
            rotations = self.get_original_rotations()
            positions = forward_rotations(
                self.skel, rotations, rtpos=self.rt_pos, trim=True
            )
    ...
    def get_velocity_factor(self):
        positions = forward_rotations(
            self.skel, self.get_original_rotations(), trim=True
        )[
            :, 1:
        ]  # [T, J - 1, 3]
    ...
    def get_BVH(self, forward=True):
        ...
        if (
            forward
        ):  # choose a direction in [z+, x+, z-, x-], which is closest to "forward", as the new z+

            directions = np.array(range(4)) * np.pi * 0.5  # [0, 1, 2, 3] * 0.5pi
            diff = (
                rt_rot[np.newaxis, :] - directions[:, np.newaxis, np.newaxis]
            )  # [1, T, 1] - [4, 1, 1]
    ...
    @classmethod
    def from_rotations_and_root_positions(
        cls, rotations, root_positions, skel=None, frametime=1 / 30
    ):
        """
        rotations: [T, J, 4]
        root_positions: [T, 3]
        ...
        foot_contact = foot_contact_from_positions(
            global_positions, fid_l=skel.fid_l, fid_r=skel.fid_r
        )
        quaters, pivots = y_rotation_from_positions(
            global_positions, hips=skel.hips, sdrs=skel.sdrs
        )
        ...
        full = np.concatenate(
            [
                rotations.reshape((len(rotations), -1)),
                root_positions,
                pivots,
                foot_contact,
            ],
    ...
    def from_BVH(cls, filename, downsample=4, skel=None, trim_scale=None):
        ...
        return cls.from_rotations_and_root_positions(
            rotations, root_positions, skel=skel, frametime=frametime * downsample
        )
...
def test_all(args):
    ...
    def test_phase_from_ft():
        ...
        right = pace[4:]
        phase_from_ft(
            np.concatenate([left, left, right, right], axis=-1), is_debug=True
        )
    ...
    def BVH_and_back(filename):
        ...
        print(
            f"content {content_input.shape}, style3d {style3d_input.shape}, style2d {style2d_input.shape}"
        )
        ...
        print(
            f"inplace no rot {inplace_no_rot.shape}, inplace {inplace.shape}, original {original.shape}"
        )
        ...
        motion_proj = {}
        for (view_point, scale, proj) in zip(
            view_points, scales, style2d_input
        ):  # [V, J * 2, T]
            proj = proj.copy().transpose(1, 0).reshape(T, -1, 2)  # [T, J, 2]
            proj = np.concatenate([proj[:, -1:], proj[:, :-1]], axis=1)
            ori_proj = np.concatenate(
                [proj[:, :1], proj[:, 1:] + proj[:, :1].copy()], axis=1
            )
            proj[:, :1] = 0
            motion_proj[
                f"angle: {(view_point[1] / np.pi * 180):3f} scale: {scale:3f}"
            ] = {"motion": ori_proj, "foot_contact": foot_contact}
            """
            visualize({
    ...
    def check_velocity(dataset):
        ...
        info = {
            content: {style: [] for style in style_names} for content in content_names
        }
